<script lang="ts">
	...
	import { checkinStatusLabel, checkinStatusVariant } from './labels';
	...
	// three panes somebody had open is not part of what they are sharing.
	let activeTab = $state('transcript');

	const recordName = $derived(detail?.project?.name ?? detail?.project?.number ?? '—');

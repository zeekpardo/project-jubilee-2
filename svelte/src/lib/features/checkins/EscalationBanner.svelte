...
{#if escalations.length > 0}
	...
	{#key conversationId}
		<section class="flex flex-col gap-3">
			{#each escalations as escalation (escalation._id)}
					...
					<div class="flex items-start gap-3">
						...
						<div class="flex min-w-0 flex-1 flex-col gap-3">
							<div class="flex flex-wrap items-center gap-2">
								...
								<!-- Category and status are separately load-bearing: WHAT was
								     matched, and whether anybody has picked it up yet. -->
								<Badge variant="destructive">
									{escalationCategoryLabel(escalation.category)}
								</Badge>
							...
							<p class="text-muted-foreground flex flex-wrap gap-x-3 gap-y-1 text-xs">
								<span>{m.escalation_matched({ term: escalation.term })}</span>
								<span>{m.escalation_turn({ number: escalation.turnNumber })}</span>
							</p>

							<!-- The family's own words, not a summary of them. -->
